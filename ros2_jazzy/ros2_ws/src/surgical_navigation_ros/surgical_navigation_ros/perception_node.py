...
def rotation_from_forward(
    ...
    forward_vector = forward_vector / norm
    ...
    if right_norm <= 0.0:
        raise ValueError(
            "Unable to construct camera right vector."
        )
    ...
    if up_norm <= 0.0:
        raise ValueError(
            "Unable to construct camera up vector."
        )
...
class TaskAwarePerceptionNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.target = SphericalStructure(
            centre=np.array(
                [0.14, 0.04, 0.00],
                dtype=float,
            ),
        ...
        self.current_pose = self._make_pose(
            position=np.array(
                [0.10, 0.10, 0.18],
                dtype=float,
            ),
    ...
    def _build_controller(
        ...
        scoring_config = ViewpointScoringConfig(
        ...
        task = SurgicalTask(
            ...
            safety_critical_points=safety_critical_points,
        )
    ...
    def _make_pose(
        ...
        rotation = rotation_from_forward(
            direction
        )
    ...
    def run_perception_cycle(self) -> None:
        """Run one task-aware viewpoint-selection cycle."""

        try:
            result = self.controller.select_viewpoint(
